...
public class ArchiveServiceEJB implements ArchiveService {
    ...
    public EventLogRecord getLogRecordWaitingForVTScan() throws ArchiveException {
        final String status = new GsonBuilder().create().toJson(VirusTotalRequestStatus.WAITING);
        final String query = "{\n" +
                "   \"query\": {\n" +
                "               \"term\": {\n" +
        ...
        final List<EventLogRecord> logRecords = elasticService.search(query, ELASTIC_LOG_INDEX + "-*", ELASTIC_LOG_TYPE, 0, 1, EventLogRecord.class);
    ...
    public EventLogRecord getLogRecordWaitingForVTReport() throws ArchiveException {
        final String status = new GsonBuilder().create().toJson(VirusTotalRequestStatus.WAITING_FOR_REPORT);
        final String query = "{\n" +
                "   \"query\": {\n" +
                "               \"term\": {\n" +
                "                   \"virus_total_request.status\": " + status + "\n" +
